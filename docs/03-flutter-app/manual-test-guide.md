...
2. **Chạy app:**
   ...
   ```powershell
   # (tùy chọn) xem id thiết bị đang chạy:
   flutter devices

   # Chạy app — DÁN NGUYÊN 1 DÒNG (đừng để bị xuống dòng):
   flutter run --dart-define=API_BASE_URL=https://eventhub-api-b4yb.onrender.com/api --dart-define=GOOGLE_SERVER_CLIENT_ID=900843335608-fr8h5ujhqk17h1v605f8dmm5nj6imk0u.apps.googleusercontent.com
   ```

   - Nếu có **nhiều thiết bị**, thêm `-d <id>` (vd `-d emulator-5554`); lấy id từ `flutter devices`.
   - Muốn xuống dòng cho dễ đọc trong **PowerShell**: thêm dấu backtick `` ` `` ở **cuối mỗi dòng** (không phải `^`/`\`).
